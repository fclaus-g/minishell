...
/*   Updated: 2023/12/22 03:26:11 by pgruz11          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_exegguttor(char *cmd, char **env)
{
	get_paths(&st, env);
	split_cmd(&st, cmd);
	find_path_index(&st, st.cmd_tab[0]);
	if (execve(st.path_cmd, st.cmd_tab, env) == -1)
	{
...
/**
 * He sacado el tema del builin exit de momento de exegguttor
 */